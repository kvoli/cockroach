...
func (s *Simulator) addStore(storeID state.StoreID, tick time.Time) {
	...
	// immediately. This is also updated via gossip, however there is a delay so
	// when calling `mma.ProcessStoreLeaseholderMsg`, there may be not be a store
	// state for some replicas. Figure this out.
