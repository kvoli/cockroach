...
type State interface {
	// Copy returns a deep copy of the current state. The State returned may be
	// modified without modifying the State with which this method was called
	// on.
	Copy() State
	...
	// Replicas returns all replias that exist on a store.
	...
	ValidTarget(RangeID, StoreID, bool) bool
	...
	// [Key, EndKey), on identical stores to the unsplit Range's replicas. This
...
// keyFmt is the formatter for representing keys as lexiographically ordered
// strings.
const keyFmt = "%010d"
