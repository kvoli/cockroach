...
func (sr *StoreRebalancer) chooseRangeToRebalance(
	...
	for {
		...
		*hottestRanges = (*hottestRanges)[1:]
		...
		targetVoterRepls, targetNonVoterRepls, foundRebalance, _ := sr.getRebalanceTargetsBasedOnQPS(
			ctx,
			rebalanceCtx,
			options,
		)
		...
		vTargets := rebalanceCtx.rangeDesc.Replicas().VoterDescriptors()
		nvTargets := rebalanceCtx.rangeDesc.Replicas().NonVoterDescriptors()

		add, remove, _, shouldRebalance := sr.rq.allocator.RebalanceTarget(
			ctx,
			rebalanceCtx.conf,
			rebalanceCtx.replWithStats.repl.RaftStatus(),
			vTargets,
			nvTargets,
			rangeUsageInfoForRepl(rebalanceCtx.replWithStats.repl),
		...
		chgs, _, err := replicationChangesForRebalance(ctx, rebalanceCtx.rangeDesc, len(vTargets), add, remove, allocatorimpl.VoterTarget)

		// chgs, _, err := replWithStats.repl.relocateOne(
		// 	ctx,
		// 	replWithStats.repl.Desc(),
		// 	roachpb.MakeReplicaSet(targetVoterRepls).ReplicationTargets(),
		// 	roachpb.MakeReplicaSet(targetNonVoterRepls).ReplicationTargets(),
		// 	false,
		// )
		if err != nil {
			log.Errorf(ctx, "unable to find first relocate target %s", err)
			return replicaWithStats{}, nil, nil, roachpb.ReplicationChanges{}
...
func (sr *StoreRebalancer) getRebalanceTargetsBasedOnQPS(
	...
	for i := 0; i < len(finalVoterTargets); i++ {
		...
		nextChgs, _, err := replicationChangesForRebalance(ctx, rbCtx.rangeDesc, len(finalVoterTargets), add,
			remove, allocatorimpl.VoterTarget)
		if err != nil {
			break
		}
		chgs = append(chgs, nextChgs...)
	...
	for i := 0; i < len(finalNonVoterTargets); i++ {
		...
		nextChgs, _, err := replicationChangesForRebalance(ctx, rbCtx.rangeDesc, len(finalNonVoterTargets), add,
			remove, allocatorimpl.NonVoterTarget)
		if err != nil {
			break
		}
		chgs = append(chgs, nextChgs...)
