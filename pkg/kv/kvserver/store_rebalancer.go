...
type LBRebalancingDimensions int64

const (
	LBRebalancingQueries LBRebalancingDimensions = iota
)

func (d LBRebalancingDimensions) toDimension() state.LoadDimension {
	switch d {
...
type RebalanceContext struct {
	...
	maxThresholds                      state.DimensionContainer
...
func (r *RebalanceContext) LessThanMaxThresholds() bool {
	return r.LocalDesc.Capacity.Dimensions().LessOrEqual(r.maxThresholds)
}
...
func (sr *StoreRebalancer) makeRebalanceContext(ctx context.Context) *RebalanceContext {
	...
	dims := LBRebalancingDimensions(LoadBasedRebalancingDimension.Get(&sr.st.SV)).toDimension()
	...
	return &RebalanceContext{
		...
		options:             options,
		mode:                LBRebalancingMode(LoadBasedRebalancingMode.Get(&sr.st.SV)),
		maxThresholds:       allocatorimpl.OverfullLoadThresholds(options.Thresholds, allStoresList.Dimensions()),
...
func (sr *StoreRebalancer) ShouldRebalanceStore(ctx context.Context, rctx *RebalanceContext) bool {
	...
	if rctx.LessThanMaxThresholds() {
		...
		log.KvDistribution.VEventf(
			...
			rctx.LocalDesc.Capacity.Dimensions(), rctx.maxThresholds, rctx.allStoresList.Dimensions())
		return false
	}
	...
	log.KvDistribution.Infof(ctx,
		...
		rctx.LocalDesc.StoreID, rctx.LocalDesc.Capacity.Dimensions(), rctx.allStoresList.Dimensions(), rctx.maxThresholds)
	return true
}
...
func (sr *StoreRebalancer) TransferToRebalanceRanges(
	...
	if !rctx.LessThanMaxThresholds() {
		log.KvDistribution.Infof(ctx,
			...
			rctx.LocalDesc.StoreID, rctx.LocalDesc.Capacity.Dimensions(),
			rctx.allStoresList.Dimensions(), rctx.maxThresholds)
		return false
	}
	...
	if rctx.mode != LBRebalancingLeasesAndReplicas {
		log.KvDistribution.Infof(ctx,
			...
			rctx.LocalDesc.Capacity.Dimensions(), rctx.maxThresholds)
		return false
	}
...
func (sr *StoreRebalancer) applyRangeRebalance(
	...
	log.KvDistribution.VEventf(
		...
		candidateReplica.RangeUsageInfo().Dimensions(),
...
func (sr *StoreRebalancer) chooseLeaseToTransfer(
	...
	for {
		...
		// TODO(kvoi): single dimension is hardcoded, handle this case.
		if candidateReplica.RangeUsageInfo().Dimensions().Dimension(rctx.LoadDimension) < rctx.LocalDesc.Capacity.Dimensions().Dimension(rctx.LoadDimension)*minLoadFraction {
			log.KvDistribution.VEventf(ctx, 3, "r%d's %s load is too little to matter relative to s%d's %s total load",
				candidateReplica.GetRangeID(), candidateReplica.RangeUsageInfo().Dimensions(), rctx.LocalDesc.StoreID, rctx.LocalDesc.Capacity.Dimensions())
			continue
		}
		...
		log.KvDistribution.VEventf(ctx, 3, "considering lease transfer for r%d with %s load",
			desc.RangeID, candidateReplica.RangeUsageInfo().Dimensions())
		...
		if targetStore, ok := rctx.storeMap[candidate.StoreID]; ok {
			log.KvDistribution.VEventf(
				...
				candidateReplica.RangeUsageInfo().Dimensions(),
				targetStore.StoreID,
				targetStore.Capacity.Dimensions(),
				rctx.LocalDesc.StoreID,
				rctx.LocalDesc.Capacity.Dimensions(),
			)
		}
...
func (sr *StoreRebalancer) chooseRangeToRebalance(
	...
	for {
		...
		// responsible for, for example, 1 load unit on a store with 5000 load units.
		const minLoadFraction = .001
		if candidateReplica.RangeUsageInfo().Dimensions().Dimension(rctx.LoadDimension) <
			rctx.LocalDesc.Capacity.Dimensions().Dimension(rctx.LoadDimension)*minLoadFraction {
			log.KvDistribution.VEventf(
				ctx,
				5,
		...
		rctx.options.PerReplicaValue = candidateReplica.RangeUsageInfo().Dimensions()
		...
		newLeaseIdx := 0
		newLeaseDimLoad := math.MaxFloat64
		...
		// TODO(kvoli): singe dimension is hard coded handle this case.
		for i := range validTargets {
			storeDesc, ok := storeDescMap[validTargets[i].StoreID]
			if ok && storeDesc.Capacity.Dimensions().Dimension(rctx.LoadDimension) < newLeaseDimLoad {
				newLeaseIdx = i
				newLeaseDimLoad = storeDesc.Capacity.Dimensions().Dimension(rctx.LoadDimension)
			}
		}
...
func (sr *StoreRebalancer) getRebalanceTargetsBasedOnLoad(
	...
	for i := 0; i < len(finalVoterTargets); i++ {
		...
		log.KvDistribution.VEventf(
			...
			rbCtx.candidateReplica.RangeUsageInfo().Dimensions(),
			rbCtx.rangeDesc.RangeID,
			remove,
			add,
		)
