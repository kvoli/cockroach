...
// TestAllocatorTransferDeterminism asserts that TransferLeaseTarget is
// deterministic when the deterministic flag is set in the storepool. It tests
// this rather narrowly by running two callers in parallel over a couple
// thousand iterations, asserting that the two callers return the same values.
//
// TODO(kvoli): TestAllocatorTransferDeterminism and
// TestAllocatorRebalanceVoterDeterminism are narrowly scoped and hand picked
// values based on a regression seen. We should run parameterized random
// testing in the simulator with a collection of actions to assert on
// determinism.
func TestAllocatorTransferDeterminism(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
...
// TestAllocatorRebalanceVoterDeterminism asserts that RebalanceVoter is
// deterministic when the deterministic flag is set in the storepool. It tests
// this rather narrowly by running two callers in parallel over a couple
// thousand iterations, asserting that the two callers return the same values.
func TestAllocatorRebalanceVoterDeterminism(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
