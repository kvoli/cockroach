...
// TestAllocatorRebalanceDeterministic tests that when there are multiple equivalence
// classes, the allocator returns a deterministic output.
func TestAllocatorRebalanceDeterministic(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	for i := 0; i < 100000; i++ {
		...
		require.Equal(t, addA, addB, "%d iters before failure", i)
		require.Equal(t, removeA, removeB, "%d iters before failure", i)
	}
}
