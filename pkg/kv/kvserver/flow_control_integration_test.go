...
-- for s3.`)
	...
	// RHS on s3.
	h.waitForConnectedStreams(ctx, right.RangeID, 2, 0 /* serverIdx */)

	h.comment(`(Sending 2x1 MiB put request to post-split LHS)`)
	h.put(ctx, k, 1, admissionpb.NormalPri)
	h.put(ctx, k, 1, admissionpb.NormalPri)
	h.comment(`(Sent 2x1 MiB put request to post-split LHS)`)

	h.comment(`(Sending 3x1 MiB put request to post-split RHS)`)
	h.put(ctx, roachpb.Key(right.StartKey), 1, admissionpb.NormalPri)
	h.put(ctx, roachpb.Key(right.StartKey), 1, admissionpb.NormalPri)
	h.put(ctx, roachpb.Key(right.StartKey), 1, admissionpb.NormalPri)
	h.comment(`(Sent 3x1 MiB put request to post-split RHS)`)

	h.waitForAllTokensReturnedForStreamsV2(ctx, 0 /* serverIdx */, testingMkFlowStream(0), testingMkFlowStream(1))
	h.waitForTotalTrackedTokens(ctx, left.RangeID, 4<<20 /* 4 MiB */, 0 /* serverIdx */)
	h.waitForSendQueueSize(ctx, left.RangeID, 3<<20 /* expSize (1+2 MiB) */, 0 /* serverIdx */)
	h.waitForTotalTrackedTokens(ctx, right.RangeID, 0 /* 0 MiB */, 0 /* serverIdx */)

	h.comment(`
-- Observe the newly split off replica, with its own two streams.
-- The third stream won't be created until the replica is caught up.`)
	h.query(n1, `
  SELECT range_id, count(*) AS streams
    FROM crdb_internal.kv_flow_control_handles_v2
GROUP BY (range_id)
...
`, "range_id", "stream_count")
	...
	h.comment(`-- Unblock admission on s3.`)
	setTokenReturnEnabled(true /* enabled */, 0, 1, 2)
	h.waitForAllTokensReturned(ctx, 3, 0 /* serverIdx */)

	h.comment(`
-- Observe the newly split off replica, with its own three streams now.`)
	h.query(n1, `
  SELECT range_id, count(*) AS streams
    FROM crdb_internal.kv_flow_control_handles_v2
GROUP BY (range_id)
...
`, "range_id", "stream_count")
	...
	h.comment(`-- Block admission on s3.`)
	setTokenReturnEnabled(false /* enabled */, 2)

	h.comment(`(Sending 4x1 MiB put request to LHS range)`)
	h.put(ctx, roachpb.Key(left.StartKey), 1, admissionpb.NormalPri)
	h.put(ctx, roachpb.Key(left.StartKey), 1, admissionpb.NormalPri)
	h.put(ctx, roachpb.Key(left.StartKey), 1, admissionpb.NormalPri)
	h.put(ctx, roachpb.Key(left.StartKey), 1, admissionpb.NormalPri)
	h.comment(`(Sent 4x1 MiB put request to LHS range)`)
...
`, "range_id", "stream_count")
	...
	h.comment(`(Sending 1x1 MiB put request to post-merged range)`)
	h.put(ctx, roachpb.Key(merged.StartKey), 1, admissionpb.NormalPri)
	h.comment(`(Sent 1x1 MiB put request to post-merged range)`)
	h.waitForAllTokensReturnedForStreamsV2(ctx, 0 /* serverIdx */, testingMkFlowStream(0), testingMkFlowStream(1))
	h.waitForTotalTrackedTokens(ctx, merged.RangeID, 4<<20 /* 4 MiB */, 0 /* serverIdx */)
