...
func (rq *replicateQueue) shouldQueue(
	...
	if !rq.store.TestingKnobs().DisableReplicaRebalancing {
		rangeUsageInfo := rq.RangeUsageInfo(repl)
...
func (rq *replicateQueue) shedLease(
	...
	if err := rq.TransferLease(ctx, repl, repl.store.StoreID(), target.StoreID, rq.RangeUsageInfo(repl)); err != nil {
...
// XXX:
func (rq *replicateQueue) RangeUsageInfo(repl *Replica) allocator.RangeUsageInfo {
	usage := RangeUsageInfoForRepl(repl)
	usage.CPUStoreNodeScale = ScaleRangeUsageInfo(rq.store.cfg.StoreLoadStatsGetter.Stats())
	return usage
}

// ScaleRangeUsageInfo returns a 
func ScaleRangeUsageInfo(storeStats load.StoreLoadStats) float64 {
